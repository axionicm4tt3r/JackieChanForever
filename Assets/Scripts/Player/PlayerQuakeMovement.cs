...
public class PlayerQuakeMovement : MonoBehaviour
{
	...
	*/
	private void AirMove()
	{
		...
		wishdir = new Vector3(playerInputVector.x, 0, playerInputVector.z);
	...
	*/
	private float CommandScale()
	{
		...
		max = (int)Mathf.Abs(playerInputVector.z);
		if (Mathf.Abs(playerInputVector.x) > max)
			max = (int)Mathf.Abs(playerInputVector.x);
		if (max <= 0)
			return 0;

		total = Mathf.Sqrt(playerInputVector.z * playerInputVector.z + playerInputVector.x * playerInputVector.x);
