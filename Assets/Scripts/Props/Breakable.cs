...
public class Breakable : MonoBehaviour, IAttackable
{
    ...
    public void ReceiveStaggerAttack(float damage, float staggerTime)
    {
        //Needs Breakable status to take damage
    }

    public void ReceiveKnockbackAttack(float damage, Vector3 direction, float knockbackVelocity, float knockbackTime)
    {
        //Needs Breakable status to take damage
        this.rigidbody.velocity = direction * knockbackVelocity / rigidbody.mass; //Something like this
    }
